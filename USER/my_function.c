...
static u8 get_checksum(u8 * p_frame, u16 frame_len)
{
	u8 sum = 0;
	u16 i = 0;
...
static void update_slave_addr(u8 addr, u8 addr_state)
{	
	u8 i = 0;
...
static void update_rfid_info(u8 addr, u8 state, u8 *rfid_id)
{
	if(state)
	{
...
static void update_back_info(u8 addr, u8 state, u8 *rfid_id)
{
	if(state)
	{
		g_slave_device_info[addr].rfid_state = RFID_INSERT;
...
static u8 rs485_read_data(u8 *recv_data)
{
	u16 recv_len = 0;
...
*/
static u8 rs485_send_cmd(u8 *cmd, u16 len)
{
...
//初始化后每5分钟广播一次
static void usart_get_slave_addr(void)
{
...
static void usart_get_slave_rfid(void)
{
	u8 i = 0;
...
//每2秒种发送一次，得知新归还的设备
static void usart_get_new_rfid_info(void)
{
...
static void rfid_ctrl_lock(uint8_t ctrl_type)
{
	if(ctrl_type == LOCK_STATE_ON) relay_on();
	else relay_off();

	g_device_info.unlock_state = ctrl_type;
	time_lock = ctrl_type;//开启5s定时器
}

static void time_out_relay_lock(void)
{
	if(time_lock >= 4500)//4.5s
	{
...
static void deal_cmd_data(struct cmd_recv_stru *p_cmd_recv_stru)
{
	if(p_cmd_recv_stru->cmd_recv_state == COM_CMD_RECV_COMPLETE)
	{
			...
			*(++p) = recv_data[4];
			switch(recv_data[4])
			{
				...
				case COM_CMD_GET_SLAVE_RFID://获取从机RFID
					*(++p) = g_device_info.rfid_state == RFID_PULL_OUT ? 0x00:0x01;
					if(*p)
					{
						memcpy(p+1,g_device_info.rfid_id,8);
						p += 8;
					}
					break;
				case COM_CMD_CTRL_SLAVE_UNLOCK://控制从机开锁
				//XX(确认字)
					if(memcmp(&recv_data[5], g_device_info.rfid_id, 8))
				...
				//XX(设备状态)  u64(64位RFID号)
					*(++p) = g_device_info.rfid_state == RFID_INSERT ? 0x01:0x00;
					if(g_device_info.rfid_state == RFID_INSERT)
						g_device_info.rfid_state = RFID_EXIST;
					if(*p)
					{
						memcpy(p+1,g_device_info.rfid_id,8);
						p += 8;
